        ...
        for symbol in ACCOUNT['symbols']:
                ...
                try:
                    ...
                    if signal and signal.growth_active and signal.growth_hyperplasia:
                        ...
                        if ratio > 0:
                            ...
                            logging.info(
                                f"[BG_CHALLENGE][{symbol}] GROWTH: hyperplasia={ratio:.0%} lot "
                                f"| signal={signal.growth_signal:.3f} "
                                f"| variant={signal.growth_variant}")
                except Exception as ge:
                    logging.debug(f"[BG_CHALLENGE] Growth hyperplasia check: {ge}")
            ...
            if self.teqa_bridge is not None:
                try:
                    ...
                    logging.debug(f"[BG_CHALLENGE] Growth lipolysis check: {le}")
...
if __name__ == "__main__":
    ...
    lock = ProcessLock("BRAIN_BG_CHALLENGE", account="365060")

    try:
        with lock:
            logging.info("=" * 60)
            logging.info("BRAIN_BG_CHALLENGE - PROCESS LOCK ACQUIRED")
            logging.info("=" * 60)
        ...
        logging.error(str(e))
        logging.error("")
        logging.error("Another instance of BRAIN_BG_CHALLENGE is already running.")
        logging.error("This prevents duplicate trades on account 365060.")
        logging.error("")
