...
# Passwords loaded from environment variables (VPS .env)
...
TRADING_ACCOUNTS = {{
    "getleveraged_1": {{
        "login": {_gl1["account"]},
        "password": os.environ.get("GL_1_PASSWORD", ""),
        "server": "{_gl1["server"]}",
        "enabled": True,
        "risk_percent": 1.0,
        "max_positions": 3
    ...
    "getleveraged_2": {{
        "login": {_gl2["account"]},
        "password": os.environ.get("GL_2_PASSWORD", ""),
        "server": "{_gl2["server"]}",
        "enabled": True,
        "risk_percent": 1.0,
        "max_positions": 3
    ...
    "getleveraged_3": {{
        "login": {_gl3["account"]},
        "password": os.environ.get("GL_3_PASSWORD", ""),
        "server": "{_gl3["server"]}",
        "enabled": True,
        "risk_percent": 1.0,
        "max_positions": 3
...
EOFCONFIG'''
        ...
        _bridge_key = os.environ.get('MT5_BRIDGE_API_KEY', '')
        if not _bridge_key:
            print("WARNING: MT5_BRIDGE_API_KEY not set in .env - generate one with: python -c \"import secrets; print(secrets.token_urlsafe(32))\"")
            _bridge_key = 'CHANGE_ME'
...
# Multi-Account Passwords (read by accounts_config.py)
GL_1_PASSWORD={_gl1["password"]}
GL_2_PASSWORD={_gl2["password"]}
GL_3_PASSWORD={_gl3["password"]}
ATLAS_PASSWORD={_atlas["password"]}

# Bridge Security
MT5_BRIDGE_API_KEY={_bridge_key}
