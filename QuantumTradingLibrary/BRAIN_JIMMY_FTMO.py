"""
QUANTUM BRAIN - JIMMY'S FTMO CHALLENGE
=======================================
DEDICATED script for Jimmy's FTMO Challenge account ONLY.
Runs independently - does NOT touch other accounts.

Account: 1512556097 (Jimmy's FTMO Challenge)
Server:  FTMO-Demo

FTMO-SPECIFIC OVERRIDES:
  - Risk auto-scales: 0.075% of account balance per trade
    (e.g. $100K -> $75/trade, $200K -> $150/trade)
  - ATR multiplier: 0.5 (wider stops, lot adjusts to maintain risk)
  These are NOT in MASTER_CONFIG.json because they only apply here.

Run: python BRAIN_JIMMY_FTMO.py
"""
...
logging.basicConfig(
    level=logging.INFO,
    format='[%(asctime)s][JIMMY] %(message)s',
    datefmt='%H:%M:%S',
    handlers=[
        logging.FileHandler('brain_jimmy_ftmo.log'),
...
# ============================================================
# FTMO-SPECIFIC OVERRIDES
# Risk scales with account size: 0.075% of balance per trade
...
def _load_account():
    """Load Jimmy's FTMO account with credentials from .env file."""
    try:
        creds = get_credentials('JIMMY_FTMO')
        return {
            'account': creds['account'],
            'password': creds['password'],
            'server': creds['server'],
            'terminal_path': creds.get('terminal_path') or r"C:\Program Files\FTMO Global Markets MT5 Terminal\terminal64.exe",
            'name': "Jimmy's FTMO Challenge",
            'symbols': creds.get('symbols', ['BTCUSD', 'XAUUSD', 'ETHUSD']),
            'magic_number': creds.get('magic', 151201),
        }
    except CredentialError as e:
        logging.error(f"Failed to load JIMMY_FTMO credentials: {e}")
        logging.error("Please configure JIMMY_FTMO_PASSWORD in .env file")
        sys.exit(1)
...
class JimmyFTMOTrader:
    ...
    def _apply_lipolysis(self, symbol: str, tighten_factor: float):
        ...
        for pos in positions:
            ...
            try:
                ...
                logging.debug(f"Lipolysis error: {e}")
    ...
    def execute_trade(self, symbol: str, action: Action) -> bool:
        ...
        # Calculate lot size to keep loss at exactly risk_per_trade
        sl_ticks = sl_distance / tick_size if tick_size > 0 else 0
        if tick_value > 0 and sl_ticks > 0:
            lot = self.risk_per_trade / (sl_ticks * tick_value)
        else:
        ...
        if result and result.retcode == mt5.TRADE_RETCODE_DONE:
            logging.info(
                f"TRADE: {action.name} {symbol} @ {price:.2f} | "
                f"Lot: {lot:.3f} | SL: ${self.risk_per_trade:.2f} | "
                f"TP: ${self.risk_per_trade * TP_MULTIPLIER:.2f} | "
                f"Dyn TP at {DYNAMIC_TP_PERCENT}%"
            )
            return True
        else:
        ...
        for symbol in ACCOUNT['symbols']:
                ...
                try:
                    ...
                    if signal and signal.growth_active and signal.growth_hyperplasia:
                        ...
                        if ratio > 0:
                            ...
                            logging.info(
                                f"[{symbol}] GROWTH: hyperplasia={ratio:.0%} lot "
                                f"| signal={signal.growth_signal:.3f} "
                                f"| variant={signal.growth_variant}")
                except Exception as ge:
                    logging.debug(f"Growth hyperplasia check: {ge}")

            # Growth Amplifier: Lipolysis
            if self.teqa_bridge is not None:
                try:
                    ...
                    logging.debug(f"Growth lipolysis check: {le}")


def main():
    print("=" * 60)
    print("  JIMMY'S FTMO CHALLENGE - QUANTUM CHILDREN")
    print(f"  Account: {ACCOUNT['account']} ({ACCOUNT['name']})")
    print(f"  Server:  {ACCOUNT['server']}")
    print(f"  Risk: 0.075% of balance | ATR mult: {FTMO_ATR_MULTIPLIER}")
    print(f"  TP: {TP_MULTIPLIER}x | Dyn TP: {DYNAMIC_TP_PERCENT}% | Rolling SL: {ROLLING_SL_MULTIPLIER}x")
    print(f"  Confidence threshold: {CONFIDENCE_THRESHOLD}")
    print("=" * 60)

    trader = JimmyFTMOTrader()
