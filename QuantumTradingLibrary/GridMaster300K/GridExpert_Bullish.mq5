//+------------------------------------------------------------------+
//|                                        GridExpert_Bullish.mq5    |
//|                              GridMaster 300K - BULLISH Expert    |
//|                       Prop Firm $300K Challenge - Conservative   |
//+------------------------------------------------------------------+
#property copyright "Quantum Trading Library"
#property version   "2.00"
#property description "BULLISH Regime Grid Expert"
...
input int      InpMagicNumber       = 300002;    // Magic Number (BULLISH)
...
{
   Print("========================================");
   Print("GRID EXPERT: BULLISH REGIME");
   ...
   if(!g_gridManager.Initialize(InpMagicNumber, _Symbol, REGIME_BULLISH))
   {
   ...
   Print("BULLISH Grid Expert initialized successfully");
...
void OnDeinit(const int reason)
{
   ...
   Print("BULLISH Grid Expert deinitialized. Reason: ", reason);
}
...
{
   //--- BULLISH expert only opens BUY orders
   int direction = 1; // BUY only
   ...
   if(g_currentGridLevel == 0)
   {
      ...
      if(g_gridManager.OpenGridOrder(direction, 1))
      {
         ...
         Print("BULLISH GRID: Opened first BUY at level 1");
      }
   }
   else
   {
      //--- Subsequent orders - check grid spacing
      double priceMove = g_lastGridPrice - currentPrice;

      //--- For BULLISH (BUY), we add more buys as price drops (better entry)
      if(priceMove >= gridSpacing)
      {
         ...
         if(nextLevel <= InpMaxOrdersPerExpert)
         {
            if(g_gridManager.OpenGridOrder(direction, nextLevel))
            {
               ...
               g_lastGridPrice = currentPrice;
               Print("BULLISH GRID: Opened BUY at level ", nextLevel);
            }
         }
      }
   }
