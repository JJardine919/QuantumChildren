...
input double   JG_ProbabilityMin = 0.10;             // G4: Min trade probability
...
input bool     UseEntropyRemoval   = true;           // Enable 12% Entropy Removal
input int      ER_Lookback         = 50;             // Price lookback for Shannon entropy
input double   ER_RemovalPct       = 0.12;           // Entropy removal factor (12%)
input double   ER_BlockLevel       = 2.20;           // Block trades above this entropy
...
public:
   ...
   bool ShouldTrade(double entropy, double interference, double confidence,
                    int direction, double amplitude_squared = 1.0)
   {
      m_total_checks++;
      m_last_block_gate = GATE_PASSED;
      m_last_block_reason = "";
...
int OnInit()
{
   ...
   if(UseJardinesGate)
   {
      ...
      Print("    G6 Kill Switch WR: ", JG_KillSwitchWR, " (", 10, " trades)");
   }
   Print("------------------------------------------------");
   Print("  12% ENTROPY REMOVAL: ", UseEntropyRemoval ? "ENABLED" : "DISABLED");
   if(UseEntropyRemoval)
   {
      Print("    Lookback: ", ER_Lookback, " bars (M1)");
      Print("    Removal Factor: ", ER_RemovalPct * 100, "%");
      Print("    Block Level: ", ER_BlockLevel);
   ...
   g_jardinesGate.SetKillSwitch(JG_KillSwitchWR, 10);
...
{
   double prices[];
   ArrayResize(prices, 50);
   ArraySetAsSeries(prices, true);

   if(CopyClose(_Symbol, PERIOD_M1, 0, 50, prices) < 50)
...
//| Uses MathLog(p)/MathLog(2) -- Shannon information entropy          |
//| Returns [0..~2.32] for 5 bins. Lower = more predictable.          |
//+------------------------------------------------------------------+
double CalculateShannonEntropy()
{
   ...
   int copied = CopyRates(_Symbol, PERIOD_M1, 0, ER_Lookback + 1, rates);
   if(copied < ER_Lookback + 1)
      return 99.0; // Cannot calculate -- return high entropy (blocks trade)
   ...
   // Bin into 5 categories (matching XAUUSD_GridCore pattern)
...
//+------------------------------------------------------------------+
//| Get Entropy Removal Factor [0..1]                                  |
//| Scales confidence down based on Shannon entropy level              |
//+------------------------------------------------------------------+
double GetEntropyRemovalFactor()
{
   ...
   double maxEntropy = MathLog(5.0) / MathLog(2.0); // ~2.322

   double normalizedEntropy = MathMin(1.0, entropy / maxEntropy);
   double factor = 1.0 - (normalizedEntropy * ER_RemovalPct);
   ...
   if(TimeCurrent() - lastLog > 300)
   {
      Print("[ENTROPY_REMOVAL] Shannon H=", DoubleToString(entropy, 4),
            " | Norm=", DoubleToString(normalizedEntropy, 4),
            " | Factor=", DoubleToString(factor, 4),
            " | Block=", (entropy >= ER_BlockLevel ? "YES" : "NO"));
      lastLog = TimeCurrent();
   }
...
//+------------------------------------------------------------------+
//| Check if Shannon entropy blocks trading                            |
//+------------------------------------------------------------------+
bool IsEntropyBlocked()
{
   ...
   double entropy = CalculateShannonEntropy();
   if(entropy >= ER_BlockLevel)
   {
      if(JG_DebugMode)
         Print("[ENTROPY_REMOVAL] BLOCKED: H=", DoubleToString(entropy, 4),
               " >= ", DoubleToString(ER_BlockLevel, 4));
      return true;
   }
   return false;
}
...
void CheckEntry()
{
   ...
   // === 12% ENTROPY REMOVAL: Block if Shannon entropy too high ===
   ...
   confidence = confidence * entropyFactor;
...
void OpenPosition(ENUM_ORDER_TYPE type, int level)
{
   ...
   if(type == ORDER_TYPE_BUY)
   {
      ...
      hiddenSL = price - (TakeProfitPts * 2.0 * point);
   }
   else
   {
      hiddenTP = price - (TakeProfitPts * point);
   ...
   for(int p = PositionsTotal() - 1; p >= 0; p--)
   {
      ...
      if(!tracked)
      {
         ...
         if(posType == POSITION_TYPE_BUY)
         {
            ...
            g_grid[idx].hiddenSL = entry - (TakeProfitPts * 2.0 * point);
         }
         else
         {
            g_grid[idx].hiddenTP = entry - (TakeProfitPts * point);
